function obj = FedAvg_AWGN_md(x,y,lr,m,id,od,pi,iter,Fop,count,epoch,batchsize,SNR,h)
...
%             delta(:,:,n) = pi(n) * localupdate(theta,x{n},y{n},epoch(n),lr,od(n),batchsize);
            delta(:,:,n) = pi(n) * localupdate(theta,x{n},y{n},epoch(n),lr/(1+0.004*t),od(n),batchsize);
        end
        if t == 1
            for n = 1:m
                normdelta(n,1) = h(n,1) / norm(delta(:,:,n));
            end
        end
        P = sqrt(id) * min(normdelta) * ones(count,iter-1);
